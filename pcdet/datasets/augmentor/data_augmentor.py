...
class DataAugmentor(object):
    ...
    def get_augmentor(self, name): 
        for cur_augmentor, cur_name in zip(self.data_augmentor_queue, self.data_augmentor_names):
            if cur_name == name:
                return cur_augmentor
        return None
    ...
    def fp_sampling(self, config=None):
        fp_sampler = database_sampler.DataBaseSampler(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger,
...
class DataAugmentors(DataAugmentor):
    """ Same as DataAugmentor but with multiple root paths.
     
        Added to support gt_sampling across multiple datasets """
    def __init__(self, root_path, root_paths, augmentor_configs, class_names, logger=None):
        self.root_paths = root_paths
        super().__init__(root_path, augmentor_configs, class_names, logger)

    def gt_sampling(self, config=None):
        db_sampler = database_sampler.DataBaseSamplers(
            root_path=self.root_path,
