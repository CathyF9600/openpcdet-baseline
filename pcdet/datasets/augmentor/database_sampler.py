...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None, sampling_type=None):
        ...
        self.gt_sampling_mode = sampler_cfg.get('GT_SAMPLING_MODE', 'fixed')
    ...
    def update_db_infos(self, buffer=0):
        ...
        for db_info_path in self.sampler_cfg.DB_INFO_PATH:
            ...
            if not db_info_path.exists():
                ...
                self.sampler_cfg.NUM_POINT_FEATURES = self.sampler_cfg.BACKUP_DB_INFO['NUM_POINT_FEATURES']
        ...
        for x in self.sampler_cfg.SAMPLE_GROUPS:
            ...
            class_name = list(x.keys())[0]
            sample_num = list(x.values())[0]
    ...
    def filter_by_min_points(self, db_infos, min_gt_points_list):
        for name_num in min_gt_points_list:
            ...
            name = list(name_num.keys())[0]
            min_num = list(name_num.values())[0]
            min_num = int(min_num)
    ...
    def sample_with_random_number(self, class_name, sample_group):
        """
        Args:
            class_name:
            sample_group:
        ...
        """
        sample_num, pointer, indices = int(sample_group['sample_num']), sample_group['pointer'], sample_group['indices']
        sample_num = np.random.randint(0, int(sample_group['sample_num']))
    ...
    def calculate_weights(self, class_scores, buffer=0, nbins=10, ratio_sampling_type=None):
        weight = np.zeros_like(class_scores, dtype=np.float64)
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                if self.gt_sampling_mode == 'random':
                    sampled_dict = self.sample_with_random_number(class_name, sample_group)
                else:
                    sampled_dict = self.sample_with_fixed_number(class_name, sample_group)

                total_gt_samples_added += len(sampled_dict)
        ...
        #print(f"Total GT samples added in this frame: {total_gt_samples_added}")
        return data_dict