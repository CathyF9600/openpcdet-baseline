...
from .augmentor.data_augmentor import DataAugmentor, DataAugmentors
from .processor.data_processor import DataProcessor
from .processor.point_feature_encoder import PointFeatureEncoder


class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None, disable_augmentation=False):
        super().__init__()

        # determine if we are loading multiple datasets
        if "DATA_PATHS" in dataset_cfg:
            print("Multiple datasets being loaded")
        ...
        if self.multiple_datasets:
            if training: 
                self.root_paths = [Path(path) for path in dataset_cfg.DATA_PATHS]
            else: 
                self.root_paths = [Path(path) for path in dataset_cfg.VAL_PATHS]
        else:
        ...
        if self.multiple_datasets:
            self.data_augmentors = [DataAugmentors(
                path, self.root_paths, dataset_cfg.DATA_AUGMENTOR, self.class_names, logger=self.logger
            ) for path in self.root_paths] if self.training else None
        else:
    ...
    def prepare_data(self, data_dict):
        ...
        Returns:
            data_dict:
                ...
                ...
        """
        if self.training and not self.disable_augmentation:
    ...
    def prepare_data_multiple(self, data_dict, root_path):
        """
        Args:
            data_dict:
                points: optional, (N, 3 + C_in)
        ...
        assert self.multiple_datasets, 'This function is only for multiple datasets'

        # determine the index of the root_path in root_paths
        root_path_idx = self.root_paths.index(root_path)
        ...
        if self.training:
            ...
            if 'calib' in data_dict:
                calib = data_dict['calib']
            data_dict = self.data_augmentors[root_path_idx].forward(
                data_dict={
                    **data_dict,
                    'gt_boxes_mask': gt_boxes_mask
                }
